...
class RSanalysis:
    ...
    def calculateHurst(self, series, exponent=None):
        ...
        limitUp = 0
        if(max(sizeRangeLog) > max(rescaledRangeLog)): limitUp = max(sizeRangeLog)
        else: limitUp = max(rescaledRangeLog)

        limitDown = 0
        if(min(sizeRangeLog) > min(rescaledRangeLog)): limitDown = min(rescaledRangeLog)
        else: limitDown = min(sizeRangeLog)
